...
For contribution uploads with attached images or videos, also set the backend-only
Cloudinary credentials from Cloudinary Console -> Dashboard -> Product Environment
Credentials:
...
CLOUDINARY_UPLOAD_FOLDER=submissions
```

Keep `CLOUDINARY_API_SECRET` only in `api/.env` and in backend deployment
environment variables. Do not expose it through any `NEXT_PUBLIC_*` frontend
variable. If the three credentials are blank, text-only submissions can still be
stored, but submissions with files will fail with `CLOUDINARY_NOT_CONFIGURED`.
