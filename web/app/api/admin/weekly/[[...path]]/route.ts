...
/** GET /api/admin/weekly  or  /api/admin/weekly/:id */
export async function GET(request: Request, { params }: Ctx) {
  ...
  return proxyDashboardRequest({ method: "GET", path: `/weekly${suffix}${search}` });
}

/** POST /api/admin/weekly — create */
export async function POST(request: Request, { params }: Ctx) {
  if ((params.path ?? []).length !== 0) {
    return Response.json({ success: false, message: "Not found." }, { status: 404 });
  }
  const body = await readJson(request);
  return proxyDashboardRequest({ method: "POST", path: "/weekly", body });
}

/** PATCH /api/admin/weekly/:id  or  /api/admin/weekly/:id/:action */
export async function PATCH(request: Request, { params }: Ctx) {
  ...
  if (segments.length === 1) {
    ...
    return proxyDashboardRequest({
      method: "PATCH",
      path: `/weekly/${encodePath(segments)}`,
      body,
    });
  }
