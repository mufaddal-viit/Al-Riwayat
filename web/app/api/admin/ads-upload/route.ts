...
/**
 * Multipart passthrough for ad creative uploads. Same trust model as the
 * weekly upload route: require the admin session cookie, attach the dashboard
 * secret server-side, and stream the image to Express, which re-validates the
 * signature before Cloudinary.
 */
export async function POST(request: Request): Promise<Response> {
  ...
  const upstream = await fetch(`${apiBaseUrl()}/admin/dashboard/ads-upload`, {
    method: "POST",
    cache: "no-store",
    headers: {
